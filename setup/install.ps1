...
$npmCacheDir = Join-Path $env:LOCALAPPDATA 'npm-cache'
if (-not (Test-Path $npmGlobalDir)) {
    Write-Host "  Creating missing npm global directory: $npmGlobalDir" -ForegroundColor Yellow
    New-Item -ItemType Directory -Path $npmGlobalDir -Force | Out-Null
}
if (-not (Test-Path $npmCacheDir)) {
    Write-Host "  Creating missing npm cache directory: $npmCacheDir" -ForegroundColor Yellow
